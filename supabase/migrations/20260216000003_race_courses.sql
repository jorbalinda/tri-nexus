...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'race_courses' AND policyname = 'Authenticated users can read global courses') THEN
    CREATE POLICY "Authenticated users can read global courses" ON race_courses FOR SELECT USING (user_id IS NULL AND auth.role() = 'authenticated');
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'race_courses' AND policyname = 'Users can read own courses') THEN
    CREATE POLICY "Users can read own courses" ON race_courses FOR SELECT USING (auth.uid() = user_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'race_courses' AND policyname = 'Users can create own courses') THEN
    CREATE POLICY "Users can create own courses" ON race_courses FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'race_courses' AND policyname = 'Users can update own courses') THEN
    CREATE POLICY "Users can update own courses" ON race_courses FOR UPDATE USING (auth.uid() = user_id);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'race_courses' AND policyname = 'Users can delete own courses') THEN
    CREATE POLICY "Users can delete own courses" ON race_courses FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;

-- Indexes (idempotent)
CREATE INDEX IF NOT EXISTS idx_race_courses_user_id ON race_courses(user_id);
CREATE INDEX IF NOT EXISTS idx_race_courses_series ON race_courses(race_series);
CREATE INDEX IF NOT EXISTS idx_race_courses_distance ON race_courses(race_distance);
