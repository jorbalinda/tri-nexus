...
import { ArrowLeft, Utensils, Zap, Droplets, FlaskConical, Trash2 } from 'lucide-react'
...
export default function NutritionClient({ race, profile }: Props) {
  const router = useRouter()
  const { plans, timeline, loading, savePlan, addTimelineEntry, removeTimelineEntry, refetch } = useNutritionPlan(race.id)
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        <button
          onClick={() => router.push(`/dashboard/races/${race.id}`)}
          className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer"
        >
          <ArrowLeft size={18} className="text-gray-500" />
        </button>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400 dark:text-gray-500 mb-1">Nutrition</p>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Race Nutrition Plan</h1>
        </div>
      </div>
      ...
      {bikePlan && (
        <div className="card-squircle p-6 sm:p-8">
          ...
          {bikeTimeline.length > 0 && (
            <div>
              <p className="text-xs font-bold uppercase tracking-[2px] text-gray-400 dark:text-gray-500 mb-3">Fueling Timeline</p>
              <div className="flex flex-col gap-2">
                {bikeTimeline.map((entry) => (
                  <div key={entry.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                    <span className="text-xs font-mono font-semibold text-blue-600 dark:text-blue-400 min-w-[60px]">{formatMinutes(entry.time_offset_minutes)}</span>
                    <span className="text-sm text-gray-700 dark:text-gray-300 flex-1">{entry.instruction}</span>
                    <button onClick={() => removeTimelineEntry(entry.id)} className="p-1 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors cursor-pointer">
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {runTimeline.map((entry) => (
                  <div key={entry.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                    <span className="text-xs font-mono font-semibold text-green-600 dark:text-green-400 min-w-[60px]">{formatMinutes(entry.time_offset_minutes)}</span>
                    <span className="text-sm text-gray-700 dark:text-gray-300 flex-1">{entry.instruction}</span>
                    <button onClick={() => removeTimelineEntry(entry.id)} className="p-1 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors cursor-pointer">
                      <Trash2 size={14} className="text-gray-400" />
                    </button>
