...
const ProjectionHistory = dynamic(() => import('@/components/races/ProjectionHistory'), {
  ssr: false,
  loading: () => <div className="card-squircle h-24 animate-pulse bg-gray-100 dark:bg-gray-800" />,
})
...
export default function RevealClient({ initialRace }: { initialRace: TargetRace }) {
  ...
  const eligibility = useMemo(() => {
    ...
    return checkRevealEligibility(initialRace, projection, workouts)
  }, [initialRace, projection, workouts])
  ...
  const racePlan = useMemo(() => {
    ...
    try {
      return generateFullRacePlan(
        workouts,
        logs,
        initialRace.race_distance as any,
        'finish',
        initialRace.race_name,
        null,
        'age_grouper',
        [],
        null,
        null,
        {
          swim: initialRace.custom_swim_distance_m,
          bike: initialRace.custom_bike_distance_km,
          run: initialRace.custom_run_distance_km,
        }
      )
    } catch {
      return null
    }
  }, [initialRace, workouts, logs])
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{initialRace.race_date}</p>
        </div>
      ...
      {tier === 0 && (
        <div className="card-squircle p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Not Enough Data for a Prediction</h2>
            ...
            <div className="max-w-sm mx-auto mb-6">
              <div className="grid grid-cols-3 gap-2 mb-4">
                {sufficiency.gates.map((gate) => (
                  <div key={gate.sport} className={`p-3 rounded-xl border text-center ${gate.passed ? 'bg-green-50/50 dark:bg-green-950/10 border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700'}`}>
                    <div className="flex items-center justify-center gap-1 mb-1">
                      {gate.passed ? <CheckCircle2 size={14} className="text-green-500" /> : <Circle size={14} className="text-gray-300 dark:text-gray-600" />}
                      <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 capitalize">{gate.sport}</span>
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{gate.workoutCount}/{gate.required} workouts</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {sufficiency && sufficiency.nextActions.length > 0 && (
            <div className="max-w-sm mx-auto text-left space-y-2">
              {sufficiency.nextActions.map((action, i) => (
                <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-xl bg-blue-50/50 dark:bg-blue-950/10 border border-blue-100 dark:border-blue-900/20">
      ...
      {tier === 1 && projection && (
        <div className="card-squircle p-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-orange-100 dark:bg-orange-950/30 text-orange-600 dark:text-orange-400 text-[10px] font-bold uppercase tracking-wider">Rough Estimate</span>
          </div>
          <p className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400 dark:text-gray-500 mb-4 text-center">Projected Finish Time</p>
          <div className="text-center mb-4">
            <p className="text-4xl font-bold text-gray-900 dark:text-gray-100">{formatTimeShort(projection.optimistic_seconds)} – {formatTimeShort(projection.conservative_seconds)}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Realistic: {formatTimeShort(projection.realistic_seconds)}</p>
          </div>
          <p className="text-xs text-center text-orange-600/80 dark:text-orange-400/80 max-w-md mx-auto">Log more data to narrow this range.</p>
        </div>
      )}
      ...
      {(tier === 2 || tier === 3) && projection && (
        <div className="card-squircle p-8">
          ...
          {tier === 3 && racePlan?.pacing_plan && (
            <div className="mt-6 border-t border-gray-100 dark:border-gray-800 pt-4">
              <p className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400 dark:text-gray-500 mb-3 text-center">Pacing Targets</p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {racePlan.pacing_plan.swim && (
                  <div className="p-3 rounded-xl bg-blue-50/30 dark:bg-blue-950/10 text-center">
                    ...
                    <p className="text-sm font-bold text-blue-600">{Math.floor(racePlan.pacing_plan.swim.targetPacePer100m / 60)}:{(racePlan.pacing_plan.swim.targetPacePer100m % 60).toString().padStart(2, '0')}/100m</p>
                  </div>
                )}
                {racePlan.pacing_plan.bike?.targetPowerRange && (
                  <div className="p-3 rounded-xl bg-orange-50/30 dark:bg-orange-950/10 text-center">
                    ...
                    <p className="text-sm font-bold text-orange-600">{racePlan.pacing_plan.bike.targetPowerRange[0]}–{racePlan.pacing_plan.bike.targetPowerRange[1]}W</p>
                  </div>
                )}
                {racePlan.pacing_plan.run && (
                  <div className="p-3 rounded-xl bg-green-50/30 dark:bg-green-950/10 text-center">
                    ...
                    <p className="text-sm font-bold text-green-600">{Math.floor(racePlan.pacing_plan.run.targetPaceSecPerKm / 60)}:{(racePlan.pacing_plan.run.targetPaceSecPerKm % 60).toString().padStart(2, '0')}/km</p>
                  </div>
                )}
              </div>
            </div>
